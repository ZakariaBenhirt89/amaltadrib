        ...
        <form action="{{ route('admin.jobs.update',$job->id) }}" method="POST" enctype="multipart/form-data" style="display: flex;flex-direction:column;gap:10px">
            ...
            <div class="row">
                <div class="form-group col-md-3">
                    ...
                    <input required type="text" id="start" name="start" class="form-control" value="{{ $job->start }}" placeholder="من..." />
                </div>
                <div class="form-group col-md-3">
                    <label for="end">إلى:</label>
                    <input required type="text" id="end" name="end" class="form-control" value="{{ $job->end }}" placeholder="إلى..." />
                </div>
                <div class="form-group col-md-3">
                ...
                <div class="form-group col-md-3">
                    ...
                    <input required type="text" id="address" name="address" class="form-control" value="{{ $job->address }}" placeholder="العنوان..." />
                </div>
                <div class="form-group col-md-3">
                    ...
                    <select required id="student" name="student" class="form-control form-select">
                        ...
                        @foreach ($students as $student)
                            <option value="{{ $student->id }}" {{ ($job->students_id == $student->id) ? 'selected="true"' : '' }}>{{ $student->id }} - {{$student->fname}}{{ $student->lname }}</option>
                        @endforeach
                    </select>
                </div>                
            </div>
